...
//	$sql = "SELECT * FROM summer_program_class_registration WHERE paid_for = '1'";
	$sql = "SELECT * FROM summer_program_class_registration WHERE";
	$db->do_query($sql);
	$regs = array();
	$x = 1;
//	$tableRows = array();
...
function showUnpaidRows(){
	if(document.getElementById('showHideRows').checked == true){
		thisDisplay = '';
	}
	else{
		thisDisplay = 'none';
	...
	for(x=0;x<unpaidRows.length;x++){
		document.getElementById(unpaidRows[x]).style.display = thisDisplay;
	}
}
</script>
<script>
