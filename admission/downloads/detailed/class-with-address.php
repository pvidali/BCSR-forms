	...
	while($row = $db->fetchObject()) {
		...
		if((isset($_REQUEST['class']) && $_REQUEST['class'] == "$class_id") || $_REQUEST['class'] == "all" || $_REQUEST['class'] == ""){
			...
			while($inner_row = $db2->fetchObject()) {
				...
				// $contact_id = $row->contact_id;
				$sql3 = "SELECT * FROM summer_program_registration WHERE contact_id = '$contact_id'";
				$db3->do_query($sql);
				while($inner_row3 = $db3->fetchObject()) {
					$this_tuition = $inner_row3->tuition;										
				}
				...
				$tableRows .= "
					<tr class=\"$rowClass\" style=\"display: $display\" id=\"$rowNum\">
						...
						<td>$permaddress_street";
						
						if(($permaddress_street2)!=""){
							$tableRows .= ", ".$permaddress_street2;
						}
						$tableRows .= "
						<br />
						$summeraddress_street";
						
						if(($summeraddress_street2)!=""){
							$tableRows .= ", ".$summeraddress_street2;
						}
						$tableRows .= "</td>
						<td>$permaddress_city<br />$summeraddress_city</td>
						<td>$permaddress_state<br />$summeraddress_state</td>
						<td>$permaddress_zip<br />$summeraddress_zip</td>
						<td>$gradelevel_2012</td>
						<td>$class_title</td>
					</tr>";
				$rowNum++;
			}
...
//	alert(classId);
	str = "class-with-address.php?class=";
	str += classId;
	window.location = str;
}
</script>
...
<table cellpadding="3" cellspacing="0">
	<tr>
		...
		<td width="195"><strong>Street Address/Summer Address</strong></td>
		<td width="95"><strong>City</strong></td>
		<td width="115"><strong>State</strong></td>
		<td width="95"><strong>Zip</strong></td>
		<td width="100"><strong>Grade Level 2012</strong></td>
		<td width="200"><strong>Class</strong></td>
	</tr>	
<?php
	echo ($tableRows);
?>
