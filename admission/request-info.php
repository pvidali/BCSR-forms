...
if(isset($_POST['submit'])) {
	...
	if($parent_url != "inquire"){
		$requiredFields = array("fname","lname","street_address","city","state","zip","email","phone","dob_month","dob_day","dob_year","high_school","high_school_state","anticipated_grad_year");
	}
	else{
		$requiredFields = array("fname","lname","street_address","city","state","zip","email","phone","dob_month","dob_day","dob_year","high_school","high_school_state","anticipated_grad_year","how_did_you_hear");
	}
    ...
    if($post_msg != ""){
		...
		$postArray['counselor'] = $fields_recruiter;
...
*/
		...
		// build the form 0 email
		$emailArray = makeEmail($postArray,$formNum,$dup_flag);
		
		// add auto-submit icontact submission form in hidden iframe
		if(!$test_env){
			$icontactFrame = makeIcontactFrame($email,$fname,$lname,$street_address,$city,$state,$zip,getDOB($dob_year,$dob_month,$dob_day),$anticipated_grad_year,$fields_recruiter);
		}
//		mail('dscheff@simons-rock.edu','icontact',$icontactFrame);
		// add to mysql db
...
//			mail("recruit@simons-rock.edu",$emailArray['subj'],$emailArray['body'],$headers);
		}
...
function timeMsg()
{
	var t=setTimeout(\"goToTYPage()\",1500);
}
timeMsg();
</script>";
	}
