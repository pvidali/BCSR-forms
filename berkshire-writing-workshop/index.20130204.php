...
if(isset($_POST['submit'])) {
	...
	if($_POST['applicant_name'] == ""){
		...
		if($newOrReturning == "returningBWW"){
			$totalTuition = "425";
		}
		else{
			$totalTuition = "475";
		...
		$sql = "INSERT INTO forms.bww_program_contact (id, date_submitted, applicant_name, permaddress_street, permaddress_street2, permaddress_city, permaddress_state, permaddress_zip,permaddress_country,permaddress_phone,cell_phone,email,hopes_info,vegetarian,living_arrangement,workshop,referral)  
		 		VALUES (NULL, now(), '$applicant_name', '$permaddress_street', '$permaddress_street2', '$permaddress_city', '$permaddress_state', '$permaddress_zip', '$permaddress_country', '$permaddress_phone', '$cell_phone', '$email', '$hopes_info', '', '', '$workshop','$referral')";
		$db->do_query($sql);
		...
		if($mop == "paypal"){
			$paybuttons = '
				...
				<input type="hidden" name="item_name" value="Berkshire Writing Workshop Fall 2012">
...
//			$workshop			= $row->workshop;
			...
			$subj = "BERKSHIRE WRITING WORKSHOP FALL 2012 REGISTRATION--PAYING BY CHECK";
			...
			// now to customer
			$msg = "";
			$msg .= "Thank you for registering for the Berkshire Writing Workshop 2012 at Simon's Rock! Payment (\$$tuitionShow)  is due in full at the time of registration. If, after registering, you discover you are unable to attend, refunds (less $50 administrative fee) will be issued for cancellations received in writing prior to September 11. After the first class meeting, students may receive a 75 percent refund if requested the next day. No refunds will be issued after September 12, 2012.\n\n";
...
<div id="innertop"></div>
	...
	<div style="clear:both;" id="student1Div">
		...
		<div style="margin: 10px 15px 10px 15px;" id="workshopDiv">
			<input class="nofloatInput" type="radio" name="workshop" id="workshopEssay" value="Essay"> 
			<label class="radiolabel" for="workshopEssay">The Personal Essay</label><br />
			<input class="nofloatInput" type="radio" name="workshop" id="workshopStory" value="Story"> 
		...
		<div id="newOrReturningDiv" style="height: 50px;">
			<div>
				...
				<div><label style="padding-top: 0; width: auto" for="returningBWW">A returning BWW student</label></div>
			</div>
			<div class="spacer"></div>
			<div>
				<div style="float:left"><input type="radio" name="newOrReturning" value="newBWW" id="newBWW" class="nofloatInput" style="width:15px"></div>
				<div><label style="padding-top: 0; width: auto" for="newBWW">A new BWW student</label></div>
			</div>
		</div>
	...
	<div style="padding: 5px 10px 10px 10px;">Payment is due in full at the time of registration. If, after registering, you discover you are unable to attend, refunds (less $50 administrative fee) will be issued for cancellations received in writing prior to September 11. After the first class meeting, students may receive a 75 percent refund if requested the next day. No refunds will be issued after September 12, 2012.</div>
