...
if(isset($_POST['submit'])) {
	...
	if($_POST['applicant_name'] == ""){
		...
		$vegetarian				= safe($_POST["vegetarian"]); 
		$living_arrangement		= safe($_POST["living_arrangement"]); 
		$workshop				= safe($_POST["workshop"]); 
		$mop					= safe($_POST["mop"]); 
		$grubstreet				= safe($_POST["grubstreet"]); 
		$simonsrock				= safe($_POST["simonsrock"]); 
		$bard					= safe($_POST["bard"]); 
		...
		else if($living_arrangement == "livingOnCampusAC"){
			$totalTuition = "1375";
		}
		else{
			$totalTuition = "1325";
		...
		// check for discounts
		if($grubstreet == "on" || $simonsrock == "on" || $bard == "on"){
			$member_discount = "yes";
		}
		
		
// 		if($member_discount == "yes" && $living_arrangement != "livingOffCampus"){
		if($member_discount == "yes"){
			$totalTuition = ($totalTuition * .9);
			$skipOtherDiscount = false; // set this back to true to exclude the double discount
		}

		if(!$skipOtherDiscount){
			$exp_date = "2012-05-16";
			...
			if ($expiration_date > $today) {
				 $totalTuition = ($totalTuition - 75);
			} 
		}
		
		// create the contact in the db
		$sql = "INSERT INTO forms.bww_program_contact (id, date_submitted, applicant_name, permaddress_street, permaddress_street2, permaddress_city, permaddress_state, permaddress_zip,permaddress_country,permaddress_phone,cell_phone,email,hopes_info,vegetarian,living_arrangement,workshop)  
		 		VALUES (NULL, now(), '$applicant_name', '$permaddress_street', '$permaddress_street2', '$permaddress_city', '$permaddress_state', '$permaddress_zip', '$permaddress_country', '$permaddress_phone', '$cell_phone', '$email', '$hopes_info', '$vegetarian', '$living_arrangement', '$workshop')";
		$db->do_query($sql);
		...
		if($mop == "paypal"){
			$paybuttons = '
				...
				<input type="hidden" name="item_name" value="Berkshire Writing Workshop 2012">
			...
			$content = "
				...
				<p style=\"padding-bottom: 12px;\">Thank you for registering for the Berkshire Writing Workshop 2012 at Simon's Rock! Payment (\$$totalTuition) is due in full at the time of registration. If, after registering, you discover you are unable to attend, refunds (lest \$75 administrative fee) will WORKSHOPrecevied in writing before <strong>June 1</strong>.</p>
				<p style=\"padding-bottom: 12px;\">If you did not print the registration form with your information on it, please print this email and include with your check payment so we may assign your payment to your registration.</p>
				<p style=\"padding-bottom: 12px;\">The total tuition due is <strong>\$$totalTuition</strong></p>
			...
			$subj = "BERKSHIRE WRITING WORKSHOP 2012 REGISTRATION--PAYING BY CHECK";
	
			$msg = "";
			$msg .= "Berkshire Writing Workshop 2012 Registration -- $type\n\n";
			$msg .= "               Tuition: \$$totalTuition\n\n";
			...
			if($vegetarian != ""){
				$msg .= "             Vegetarian meals: $vegetarian\n\n";
			}
			...
			// now to customer
			$msg = "";
			$msg .= "Thank you for registering for the Berkshire Writing Workshop 2012 at Simon's Rock! Payment (\$$totalTuition) is due in full at the time of registration. If, after registering, you discover you are unable to attend, refunds (lest \$75 administrative fee) will WORKSHOPreceived in writing before June 1.\n\n";
			...
			$msg .= "Please make checks payable to: ???\n\n";
			$msg .= "Mail/deliver to:\n";
			...
			mail($to,"Berkshire Writing Workshop 2012 at Simon's Rock",$msg, "From: alobron@simons-rock.edu");
			mail("dscheff@simons-rock.edu","COPY (orig: $to): Berkshire Writing Workshop 2012 at Simon's Rock",$msg, "From: alobron@simons-rock.edu");
		}
	}
}
...
<html><head>
<meta charset="utf-8">
<title>Berkshire Writing Workshop 2012 at Simon's Rock Registration</title>
...
<div id="innertop"></div>
	...
	<div style="clear:both;" id="student1Div">
		...
		<div style="margin: 10px 15px 10px 15px;" id="workshopDiv">
			<input class="nofloatInput" type="radio" name="workshop" id="workshopFiction" value="Fiction"> 
			<label class="radiolabel" for="workshopFiction">Fiction with Brendan Mathews</label><br />
			<input class="nofloatInput" type="radio" name="workshop" id="workshopMemoir" value="Memoir"> 
			<label class="radiolabel" for="workshopMemoir">Memoir with Alison Lobron</label>
		</div>
        <div class="spacer"></div>
		<div>Please use this space to tell us your hopes for the workshop, and any additional information you'd like us to know (in 200 words or fewer please.)</div>
		<div>
		...
		<div class="spacer" style="height: 20px;"><strong>Meals: </strong></div>
		<div>
			<div style="float:left"><input type="checkbox" name="vegetarian" value="on" id="vegetarian" class="nofloatInput" style="width:15px"> </div>
			<div><label style="padding-top: 0; width: auto" for="vegetarian">I prefer vegetarian meals.</label></div>
		</div>
		<div class="spacer" style="height: 15px;"></div>
		<div class="spacer" style="height: 20px;"><strong>Are you: </strong></div>
		<div>
			<div style="float:left"><input type="checkbox" name="grubstreet" value="on" id="grubstreet" class="nofloatInput" style="width:15px"></div>
			<div><label style="padding-top: 0; width: auto" for="grubstreet">A member of Grub Street</label></div>
		</div>
        <div class="spacer"></div>
		<div>
			<div style="float:left"><input type="checkbox" name="simonsrock" value="on" id="simonsrock" class="nofloatInput" style="width:15px"></div>
			<div><label style="padding-top: 0; width: auto" for="simonsrock">Faculty/Staff/Alum of Simon's Rock</label></div>
		</div>
        <div class="spacer"></div>
		<div>
			<div style="float:left"><input type="checkbox" name="bard" value="on" id="bard" class="nofloatInput" style="width:15px" onClick="toggleDiv('bardDetailsDiv')"></div>
			<div><label style="padding-top: 0; width: auto" for="bard">Faculty/Staff/Alum of another Bard College program</label></div>
		</div>
		<div style="display: none" id="bardDetailsDiv">
			<div class="spacer"></div>
			<div>Please tell us of your other Bard affiliation: <input type="text" id="bard_details"></div>
		</div>
		...
		<div style="margin: 5px 15px 0 15px;" id="living_arrangementDiv">
			...
			<label class="radiolabel" for="livingOffCampus">Off-Campus: ($1,025)</label><br />
			<input class="nofloatInput" type="radio" name="living_arrangement" id="livingOnCampusAC" value="livingOnCampusAC"> 
			<label class="radiolabel" for="livingOnCampusAC">On-Campus, and I must have A/C: ($1,375)</label><br />
			<input class="nofloatInput" type="radio" name="living_arrangement" id="livingOnCampus" value="livingOnCampus"> 
			<label class="radiolabel" for="livingOnCampus">On-Campus, and I'm OK with or without A/C: ($1,325)</label>
		</div>
		
        <div class="spacer"></div>
		<div>How did you hear about the Berkshire Writing Workshop?</div>
		<div>
	...
	<div style="padding: 5px 10px;">Payment is due in full at the time of registration. If, after registering, you discover you are unable to attend, refunds (less $100 administrative fee) will be issued for cancellations received in writing by <strong>June 1</strong>.</div>
...
//			alert ("Please select a workshop");
		...
		if(!getCheckedValue(document.request.living_arrangement)){
            rval = false
			fieldFocus = "living_arrangementDiv";
//			alert ("Please select a workshop");
			if(fieldFocus != "") {
				document.getElementById(fieldFocus).style.display='';
				document.getElementById(fieldFocus).style.backgroundColor = highlight
