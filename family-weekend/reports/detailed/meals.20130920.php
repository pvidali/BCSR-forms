...
<?php 		
	...
	$sql = "SELECT * FROM fw_program_meal_registration WHERE date_submitted > '$rangestart' AND date_submitted < '$rangeend' AND paid_for = '1' ORDER BY date_submitted";
	$db->do_query($sql);
	while($row = $db->fetchObject()) {
		...
		$mealsArray = explode("|",$meals);  // this is ugly... but since we know the order they are coming in because we pushed them in. 
	...
	foreach($allMeals as $mealKey => $mealValue){
		...
		if(count($mealValue)){
			$FRLTable .= "<tr class=columnheader><td style='min-width: 250px; white-space: nowrap'>Registrant</td><td>Meals purchased</td></tr>";
		}
		else {
...
<ul>
	...
	<li><a href="javascript: showTable('STL')">Saturday Lunch</a></li>
