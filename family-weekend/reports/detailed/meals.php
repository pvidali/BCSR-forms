...
//	$STL = array();
	...
	$sql = "SELECT * FROM fw_program_meal_registration_2014 WHERE date_submitted > '$rangestart' AND date_submitted < '$rangeend' ORDER BY date_submitted";
	$db->do_query($sql);
	while($row = $db->fetchObject()) {
		...
		/* 	This is ugly... but since we know the order they are coming in because we pushed them in, we'll blindly rely on that. Sorry!
...
//	$allMealsNames = array('FRL' => "Friday Lunch",'FRD' => "Friday Dinner",'STB' => "Saturday Breakfast",'STL' => "Saturday Lunch",'STD' => "Saturday Dinner",'SNB' => "Sunday Brunch");
	$allMeals = array('FRL' => $FRL,'FRD' => $FRD,'STB' => $STB,'STL' => $STL,'STD' => $STD,'SNB' => $SNB);
	$allMealsNames = array('FRL' => "Friday Lunch",'FRD' => "Friday Dinner",'STB' => "Saturday Breakfast",'STL' => "Saturday Harvest Lunch",'STD' => "Saturday Dinner",'SNB' => "Sunday Brunch");
	...
	foreach($allMeals as $mealKey => $mealValue){
		...
		foreach($mealValue as $k => $v){
			$sql = "SELECT * FROM family_weekend_2014 WHERE id = $k";
			$db->do_query($sql);
			$res = $db->fetchObject();
	...
	//		$FRLTable = "PAYER ID: $k, NUMER RESERVED: $v";
			...
			if($v == 0){
				...
				if($PMT_STATUS[$k] == 0){
					$pmt_status_show_as = "not paid";
				}
				else {
					$pmt_status_show_as = "paid";
				}
				$FRLTableRow .= $pmt_status_show_as; // $PMT_STATUS[$k];
			}
			$FRLTableRow .= "</td></tr>";
			$thiskey = $lname.$fname;
			$x = 0;
...
<ul>
	...
	<li><a href="javascript: showTable('STB')">Saturday Breakfast</a></li>
	<li><a href="javascript: showTable('STL')">Saturday Harvest Lunch</a></li>
