...
if ($verified) {
	...
	$sql = "INSERT INTO forms.fw_meal_payments VALUES (NULL, '$txn_id', '$payer_email', '$mc_gross')";
	$db->do_query($sql);

	$payer_id = mysql_real_escape_string($_POST['custom']);
	$sql = "UPDATE fw_program_meal_registration SET paid_for='1' WHERE payer_id='$payer_id'";
	$db->do_query($sql);
	
	// mail to Admin
	$sql = "SELECT * FROM forms.family_weekend WHERE id='$payer_id' LIMIT 1";
	$db->do_query($sql);
	$row = $db->fetchObject();
	...
	$meals .= "\n";
	$meals .= "Saturday brunch and Harvest Fest Lunch: ";
	$meals .= $row->saturdayBrunchAdults . "@ \$9: \$" .($row->saturdayBrunchAdults*9);
	$meals .= "\n";
