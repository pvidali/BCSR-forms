...
function iw_fieldmap_name($db,$fieldname){
	...
	if($row){
		...
		return $row; // this should be returning false
	}
	//	$iw_naming = array();
...
function get_recruiter($zip, $state, $country, $db) {
	// #20130918 @dscheff - to replace getTerritoryInfoDB and getTerritoryInfo, use zip if available
...
function getTerritoryInfoDB($state,$country,$db){
	// depredated - use get_recruiter instead
	...
	$territory = $state;
	if ($territory == "") {
		// HACK - no country, set to a randonm one to show them Chandra
		$territory = "CHAD";
//		$territory = $country;
	}
...
function getTerritoryInfoDB_TRY($state,$country,$db){
	// depredated - use get_recruiter instead
